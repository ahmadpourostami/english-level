...
class English_Level_Assessment {
    ...
    private function clean_choice($value,$allowed,$fallback){$value=sanitize_text_field(wp_unslash($value??''));return in_array($value,$allowed,true)?$value:$fallback;}
    ...
    public function admin_page(){
        ...
        $paged=max(1,absint($_GET['paged']??1));$per_page=20;$where=[];$params=[];
        if($level){$where[]='level=%s';$params[]=$level;} if($skill){$where[]='skill=%s';$params[]=$skill;}
        if($search){$where[]='question LIKE %s';$params[]='%'.$wpdb->esc_like($search).'%';}
        $where_sql=$where?' WHERE '.implode(' AND ',$where):'';
        $total=$params?$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $table$where_sql",$params)):$wpdb->get_var("SELECT COUNT(*) FROM $table");
        $offset=($paged-1)*$per_page;$list_params=$params; $list_params[]=$per_page;$list_params[]=$offset;
        $qs=$wpdb->get_results($wpdb->prepare("SELECT * FROM $table$where_sql ORDER BY id DESC LIMIT %d OFFSET %d",$list_params));
        $pages=max(1,(int)ceil($total/$per_page));
        $level_counts=$wpdb->get_results("SELECT level,COUNT(*) total FROM $table GROUP BY level");$level_map=[];foreach($level_counts as $r)$level_map[$r->level]=(int)$r->total;
        $skill_counts=$wpdb->get_results("SELECT skill,COUNT(*) total FROM $table GROUP BY skill");$skill_map=[];foreach($skill_counts as $r)$skill_map[$r->skill]=(int)$r->total;
        ...
        <div class="ela-admin-form"><h2><?php echo $editing?'ویرایش سؤال #'.(int)$editing->id:'افزودن سؤال';?></h2><form method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>"><input type="hidden" name="action" value="ela_save_question"><input type="hidden" name="id" value="<?php echo $editing?(int)$editing->id:0;?>"><?php wp_nonce_field('ela_save_question');?><p><textarea name="question" rows="3" class="large-text" required><?php echo $editing?esc_textarea($editing->question):'';?></textarea></p><div class="ela-row"><?php foreach(['a','b','c','d'] as $letter):$field='option_'.$letter;?><p><label><strong><?php echo strtoupper($letter);?></strong></label><input type="text" name="<?php echo esc_attr($field);?>" value="<?php echo $editing?esc_attr($editing->$field):'';?>" required></p><?php endforeach;?></div><div class="ela-row"><p><label>Correct answer</label><select name="correct_answer"><?php foreach(['A','B','C','D'] as $c):?><option value="<?php echo $c;?>" <?php selected($editing?$editing->correct_answer:'A',$c);?>><?php echo $c;?></option><?php endforeach;?></select></p><p><label>Level</label><select name="level"><?php foreach($this->levels as $l):?><option value="<?php echo $l;?>" <?php selected($editing?$editing->level:'A1',$l);?>><?php echo $l;?></option><?php endforeach;?></select></p><p><label>Skill</label><select name="skill"><?php foreach($this->skills as $s):?><option value="<?php echo $s;?>" <?php selected($editing?$editing->skill:'grammar',$s);?>><?php echo ucfirst($s);?></option><?php endforeach;?></select></p></div><p><label>Points</label> <input type="number" name="points" min="1" max="10" value="<?php echo $editing?max(1,(int)$editing->points):1;?>" style="width:90px"></p><?php submit_button($editing?'Update Question':'Add Question');?> <?php if($editing):?><a class="button" href="<?php echo esc_url(admin_url('admin.php?page=ela-questions'));?>">Cancel edit</a><?php endif;?></form></div>
        <h2>Question Bank</h2><form class="ela-filter" method="get"><input type="hidden" name="page" value="ela-questions"><input type="search" name="s" value="<?php echo esc_attr($search);?>" placeholder="Search questions..."><select name="level"><option value="">All levels</option><?php foreach($this->levels as $l):?><option value="<?php echo $l;?>" <?php selected($level,$l);?>><?php echo $l;?></option><?php endforeach;?></select><select name="skill"><option value="">All skills</option><?php foreach($this->skills as $s):?><option value="<?php echo $s;?>" <?php selected($skill,$s);?>><?php echo ucfirst($s);?></option><?php endforeach;?></select><button class="button button-primary">Filter</button> <a class="button" href="<?php echo esc_url(admin_url('admin.php?page=ela-questions'));?>">Reset</a></form>
        <p><strong>Skills:</strong> <?php foreach($this->skills as $s)echo esc_html(ucfirst($s)).': '.(int)($skill_map[$s]??0).' &nbsp; ';?></p><table class="widefat striped"><thead><tr><th>ID</th><th>Question</th><th>Level</th><th>Skill</th><th>Points</th><th>Correct</th><th>Actions</th></tr></thead><tbody><?php if($qs):foreach($qs as $q):?><tr><td><?php echo(int)$q->id;?></td><td class="ela-question-cell"><?php echo esc_html($q->question);?></td><td><strong><?php echo esc_html($q->level);?></strong></td><td><?php echo esc_html(ucfirst($q->skill));?></td><td><?php echo(int)$q->points;?></td><td><?php echo esc_html($q->correct_answer);?></td><td class="ela-actions"><a href="<?php echo esc_url(add_query_arg(['page'=>'ela-questions','edit'=>$q->id],admin_url('admin.php')));?>">Edit</a> | <a class="ela-danger" onclick="return confirm('Delete this question?');" href="<?php echo esc_url(wp_nonce_url(admin_url('admin-post.php?action=ela_delete_question&id='.$q->id),'ela_delete_'.$q->id));?>">Delete</a></td></tr><?php endforeach;else:?><tr><td colspan="7">No questions found.</td></tr><?php endif;?></tbody></table>
        <?php if($pages>1)echo '<div style="margin-top:16px">'.wp_kses_post(paginate_links(['base'=>add_query_arg(['page'=>'ela-questions','paged'=>'%#%','s'=>$search,'level'=>$level,'skill'=>$skill],admin_url('admin.php')),'current'=>$paged,'total'=>$pages,'type'=>'plain'])).'</div>'; ?>
        <h2>Latest Results</h2><table class="widefat striped"><thead><tr><th>User</th><th>Score</th><th>Level</th><th>Date</th></tr></thead><tbody><?php foreach($results as $r):?><tr><td><?php echo $r->user_id?(int)$r->user_id:'Guest';?></td><td><?php echo(int)$r->score;?>%</td><td><?php echo esc_html($r->level);?></td><td><?php echo esc_html($r->created_at);?></td></tr><?php endforeach;?></tbody></table></div><?php
    }

    public function save_question(){
        if(!current_user_can('manage_options')||!check_admin_referer('ela_save_question'))wp_die('Unauthorized');
        global $wpdb;$table=$wpdb->prefix.'ela_questions';$id=absint($_POST['id']??0);
        $data=['question'=>sanitize_textarea_field(wp_unslash($_POST['question']??'')),'option_a'=>sanitize_text_field(wp_unslash($_POST['option_a']??'')),'option_b'=>sanitize_text_field(wp_unslash($_POST['option_b']??'')),'option_c'=>sanitize_text_field(wp_unslash($_POST['option_c']??'')),'option_d'=>sanitize_text_field(wp_unslash($_POST['option_d']??'')),'correct_answer'=>$this->clean_choice($_POST['correct_answer']??'A',['A','B','C','D'],'A'),'level'=>$this->clean_choice($_POST['level']??'A1',$this->levels,'A1'),'skill'=>$this->clean_choice($_POST['skill']??'grammar',$this->skills,'grammar'),'points'=>max(1,min(10,absint($_POST['points']??1)))];
